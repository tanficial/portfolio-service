...
import { currentDateValidation, startToEndDateValidation } from "utils/validation";
...
const EditProjectForm = ({ project, index, onSave, onDelete }) => {
    ...
    const handleProjectSubmit = e => {
        ...
        if (title && detail && startDate && endDate) {
            ...
            if (!currentDateValidation(startDate) || !currentDateValidation(endDate)) {
    ...
    return (
        <PortfolioEdit.PortfolioEditForm onSubmit={handleProjectSubmit}>
            ...
            <PortfolioEdit.DatePickerWrapper>
                <input type="date" name="start_date" value={startDate} max={new Date(endDate) < new Date() ? endDate : dateToString(new Date())} onChange={e => setStartDate(e.target.value)} />
                <span>~</span>
                <input type="date" name="end_date" value={endDate} min={startDate} max={dateToString(new Date())} onChange={e => setEndDate(e.target.value)} />
            </PortfolioEdit.DatePickerWrapper>
...
const ProjectCard = ({ projects, loginId, setLoginId, isEditAble, setPortfolio }) => {
    ...
    const saveProject = async (itemId, index, title, detail, startDate, endDate) => {
        try {
            if (itemId) {
                await projectApi.updateProjectRequest(itemId, title, detail, startDate, endDate);
            }
            else {
                const newItemId = await projectApi.addProjectRequest(loginId, title, detail, startDate, endDate);
                setPortfolio(produce(draft => {
            ...
            <h2>프로젝트</h2>
            {
                edit
                    ...
                    : projects.map(project => {
                        return (
                            <PortfolioCard.PortfolioElement key={`project-list-${project.id}`}>
                                <p>{project.title}</p>
                                <p>{project.detail}</p>
                ...
                isEditAble && (
                    <PortfolioCard.ButtonWrapper>
                        {edit
                            ? (
                                <>
                                    <PortfolioCard.Button onClick={() => addProjectForm()}>추가</PortfolioCard.Button>
                                    <PortfolioCard.Button onClick={() => editProjectComplete()}>수정완료</PortfolioCard.Button>
                                </>
                            )
