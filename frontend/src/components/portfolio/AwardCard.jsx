...
const EditAwardForm = ({ award, index, onSave, onDelete }) => {
    ...
    return (
        <PortfolioEdit.PortfolioEditForm onSubmit={handleAwardSubmit}>
            <input type="text" name="title" value={title} onChange={e => setTitle(e.target.value)} placeholder="수상내역" />
            <input type="text" name="detail" value={detail} onChange={e => setDetail(e.target.value)} placeholder="상세내역" />
...
const AwardCard = ({ awards, loginId, setLoginId, isEditAble, setPortfolio }) => {
    ...
    const saveAward = async (itemId, index, title, detail) => {
        try {
            if (itemId) {
                ...
                const newItemId = await awardApi.addAwardRequest(loginId, title, detail);
                setPortfolio(produce(draft => {
                    draft.awards[index] = {
                        id: newItemId,
                        title,
                ...
                isEditAble && (
                    <PortfolioCard.ButtonWrapper>
                        {edit
                            ? (
                                <>
                                    <PortfolioCard.Button onClick={() => addAwardForm()}>추가</PortfolioCard.Button>
                                    <PortfolioCard.Button onClick={() => editAwardComplete()}>수정완료</PortfolioCard.Button>
                                </>
                            )
